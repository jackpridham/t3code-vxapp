...
function formatSchemaDecodeIssue(error: unknown): string {
  const issue = (error as { readonly issue?: unknown } | null)?.issue;
  if (issue !== undefined) {
    try {
      return SchemaIssue.makeFormatterDefault()(issue as never);
    } catch {
      // Fall through to the thrown error message. Effect v4 decode helpers can
      // throw plain Error objects instead of the SchemaError shape.
    }
  }
...
export function toOrchestrationCommandDecodeError(error: unknown) {
  return new OrchestrationCommandDecodeError({
    issue: formatSchemaDecodeIssue(error),
    cause: error,
  });
}
