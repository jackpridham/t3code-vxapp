...
export function DevOrchestrationMenu({
  ...
  return (
    <div className="border-b border-dashed border-amber-500/35 bg-amber-500/5 px-3 py-2 sm:px-5">
      ...
      >
        <div className="mx-auto w-full max-w-3xl rounded-lg border border-amber-500/25 bg-background/80 p-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="h-5 text-[10px]">
                  {programTargets.length} program{" "}
                  {programTargets.length === 1 ? "target" : "targets"}
                </Badge>
                <Badge variant="outline" className="h-5 text-[10px]">
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                Uses real orchestration commands. Intended for manual dev/test flows.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <CollapsibleTrigger
                render={
                  <Button
                    ...
                    aria-label={
                      detailsOpen
                        ? "Hide dev orchestration details"
                        : "Show dev orchestration details"
                    }
                  />
                }
              >
              ...
              <Menu>
                ...
                <MenuPopup align="end" className="max-w-96">
                  ...
                  {programTargets.length > 0 ? (
                    <>
                      ...
                      {programTargets.map((target) => (
                        <Fragment key={`program:${target.programId}`}>
                          <MenuGroupLabel className="pt-0 text-[10px] text-foreground">
                            {target.programTitle}
                          </MenuGroupLabel>
                          ...
                          {DEV_PROGRAM_NOTIFICATION_KIND_SECTIONS.map((section) => (
                            <div key={`${target.programId}:${section.id}`}>
                              <MenuGroupLabel className="pt-1 text-[10px]">
                                {section.label}
                              </MenuGroupLabel>
                  ...
                  {orchestratorTargets.length > 0 ? (
                    <>
                      ...
                      {orchestratorTargets.map((target) => (
                        <Fragment key={`wake:${target.orchestratorThreadId}`}>
                          <MenuGroupLabel className="pt-0 text-[10px] text-foreground">
                          ...
                          <MenuGroupLabel className="pt-1 text-[10px]">Wake outcome</MenuGroupLabel>
