...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const chatHeaderBadgeLabel = resolveChatHeaderBadgeLabel({
    activeThread,
    activeProjectName: activeProject?.name,
  });
  const pageTitle = resolveChatDocumentTitle({
    thread: activeThread,
  ...
  const isComposerAutoHiddenForScroll =
    chatViewInputWhenScrolling === "hide" && showScrollToBottom && !isComposerRevealedWhileScrolled;
  const isComposerHidden = isComposerManuallyHidden || isComposerAutoHiddenForScroll;
  // In compact mode, collapse agentic controls while scrolled away from the bottom
  // unless the composer already has content the user may want to adjust before sending.
  ...
  useEffect(() => {
    if (!showScrollToBottom || chatViewInputWhenScrolling !== "hide") {
      setIsComposerRevealedWhileScrolled(false);
    }
  }, [chatViewInputWhenScrolling, showScrollToBottom]);
  useEffect(() => {
    setIsComposerRevealedWhileScrolled(false);
  }, [activeThread?.id]);
  useEffect(() => {
  ...
  const completionDuration = useMemo(() => {
    ...
    return formatElapsed(activeLatestTurn.startedAt, activeLatestTurn.completedAt);
  }, [
    activeLatestTurn?.completedAt,
  ...
  const showChangesDrawerToggle = true;
  ...
  const showComposer = useCallback(() => {
    if (chatViewInputWhenScrolling === "hide" && showScrollToBottom) {
      setIsComposerRevealedWhileScrolled(true);
    }
    setIsComposerManuallyHidden(false);
  }, [chatViewInputWhenScrolling, showScrollToBottom]);
  const hideComposer = useCallback(() => {
    setIsComposerInputFocused(false);
    if (chatViewInputWhenScrolling === "hide" && showScrollToBottom && !isComposerManuallyHidden) {
      setIsComposerRevealedWhileScrolled(false);
      return;
    }
    setIsComposerManuallyHidden(true);
  }, [chatViewInputWhenScrolling, isComposerManuallyHidden, showScrollToBottom]);
  const toggleComposerVisibility = useCallback(() => {
    ...
    hideComposer();
  }, [hideComposer, isComposerHidden, showComposer]);
  useEffect(() => {
    return () => {
      cancelPendingStickToBottom();
      cancelPendingInteractionAnchorAdjustment();
            ...
            <div className="pointer-events-none absolute bottom-1 right-3 z-30 py-1.5 sm:right-5">
              <Tooltip>
                <TooltipTrigger
                  render={
                    <button
                      ...
                      aria-label={isComposerHidden ? "Show chat input" : "Hide chat input"}
                      className="pointer-events-auto flex size-8 items-center justify-center rounded-full border border-border/60 bg-card text-muted-foreground shadow-sm transition-colors hover:border-border hover:bg-card/95 hover:text-foreground hover:cursor-pointer"
                    />
                  }
                >
                  {isComposerHidden ? (
                ...
                <TooltipPopup side="left">
                  {isComposerHidden ? "Show chat input" : "Hide chat input"}
                </TooltipPopup>
              </Tooltip>
            </div>

            {/* Floating scroll controls — shown when user has scrolled away from the bottom */}
            {showScrollToBottom && (
              <div className="pointer-events-none absolute bottom-1 left-1/2 z-30 flex -translate-x-1/2 justify-center gap-2 py-1.5">
                <button
                  type="button"
