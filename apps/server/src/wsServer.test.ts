...
describe("WebSocket Server", () => {
  ...
  async function seedBootstrapSpecialProject(input: {
    ...
    if (input.kind === "orchestrator") {
      const archiveOlderThreadResponse = await sendRequest(
        input.ws,
        ORCHESTRATION_WS_METHODS.dispatchCommand,
        {
          type: "thread.archive",
          commandId: `cmd-${input.olderThreadId}-archive`,
          threadId: input.olderThreadId,
        },
      );
      expect(archiveOlderThreadResponse.error).toBeUndefined();
    }
  ...
  it("returns a normalized worker runtime snapshot for selected worker threads", async () => {
    ...
    mockedWorkerRuntimeSnapshot.mockResolvedValueOnce({
      ...
      reasonCode: null,
      runtimeDir,
      runtimeRoot: path.join(worktreePath, ".agents"),
    ...
    expect(runtimeResponse.result).toEqual(
      expect.objectContaining({
        ...
        audit: expect.objectContaining({ status: "error" }),
      ...
      issues: [
        {
          ...
          detail: "Required runtime input is missing.",
          path: path.join(runtimeDir, "dispatch-contract.json"),
          runtimeFile: "dispatch-contract.json",
          severity: "blocker",
        },
      ],
    } as Awaited<ReturnType<typeof fetchAgentsVxappWorkerRuntimeSnapshot>>);
  ...
  it("returns an unavailable worker runtime snapshot for non-worker threads", async () => {
    ...
    mockedWorkerRuntimeSnapshot.mockResolvedValueOnce({
      ...
      sourceFiles: {
        contextPlan: {
          status: "missing",
          failureCode: "runtime_authority_missing",
          failureMessage: "Thread 'thread-orchestrator-runtime' is not a worker thread.",
        },
        dispatchContract: {
          status: "missing",
          failureCode: "runtime_authority_missing",
  ...
  it("falls back to the worker project root when a worker thread has no worktree path", async () => {
    ...
    mockedWorkerRuntimeSnapshot.mockResolvedValueOnce({
      ...
      reasonCode: "runtime_files_missing",
      runtimeDir,
      runtimeRoot: path.join(projectRoot, ".agents"),
