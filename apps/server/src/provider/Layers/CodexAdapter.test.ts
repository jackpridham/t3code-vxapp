...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps raw completed reasoning items to canonical reasoning deltas", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-raw-reasoning-complete"),
        kind: "notification",
        provider: "codex",
        createdAt: new Date().toISOString(),
        ...
        payload: {
          ...
          item: {
            ...
            summary: [
              {
                type: "summary_text",
                text: "Checked the arithmetic mentally.",
              },
            ],
            content: [],
          },
      ...
      assert.equal(firstEvent.value.payload.delta, "Checked the arithmetic mentally.");
    }),
  );
