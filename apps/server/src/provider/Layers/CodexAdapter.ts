...
function extractRawResponseReasoningContent(
  payload: EffectCodexSchema.V2RawResponseItemCompletedNotification,
):
  | {
      readonly streamKind: "reasoning_summary_text" | "reasoning_text";
  ...
  const summaryDelta = payload.item.summary
  ...
  const contentDelta = (payload.item.content ?? [])
    .map((part) => trimText(part.text))
    .filter((part): part is string => part !== undefined)
    .join("\n\n");
...
function mapToRuntimeEvents(
  ...
  if (event.method === "rawResponseItem/completed") {
    ...
    const reasoningContent = payload ? extractRawResponseReasoningContent(payload) : undefined;
    if (!reasoningContent) {
      return [];
    }
    return [
