...
const fakeOllamaAdapter: OllamaAdapterShape = {
  ...
  capabilities: { sessionModelSwitch: "in-session", sessionRecovery: "history-replay" },
  startSession: vi.fn(),
  sendTurn: vi.fn(),
  interruptTurn: vi.fn(),
  respondToRequest: vi.fn(),
...
layer("ProviderAdapterRegistryLive", (it) => {
  it.effect("resolves a registered provider adapter", () =>
    Effect.gen(function* () {
      ...
      const ollama = yield* registry.getByProvider("ollamaLocal");
      assert.equal(codex, fakeCodexAdapter);
      assert.equal(claude, fakeClaudeAdapter);
      assert.equal(ollama, fakeOllamaAdapter);

      const providers = yield* registry.listProviders();
      assert.deepEqual(providers, ["codex", "claudeAgent", "ollamaLocal"]);
    }),
  );
